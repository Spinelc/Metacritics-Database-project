...
CREATE TABLE genre_table (
    ...
    genre VARCHAR(19) -- the column genre in sales had a maximum lenght of 19.
);
...
CREATE TABLE reviewer_table(
    ...
    reviewer VARCHAR(254), -- Out of the reviewer names of experts and users, the maximum character length is 254.
    role_id INT,
...
CREATE TABLE review_table(
    ...
    positive_emotion FLOAT(8), -- positive emotions column in expert_reviews ranges from -1 to +1.
    negative_emotion FLOAT(8), -- negative emotions column in expert_reviews ranges from -1 to +1.
    reviewer_id INT,

    FOREIGN KEY(reviewer_id)
        REFERENCES reviewer_table(reviewer_id)
...
CREATE TABLE movie_table(  -- Carola
    ...
    meta_score INT,
    user_score FLOAT(8), -- user score column in meta has a small range of decimal numbers.
    production_budget INT,
    world_wide_box_office INT,
    rating_id INT,
    review_id INT,

    FOREIGN KEY(rating_id)
        REFERENCES age_rating_table(rating_id),

    FOREIGN KEY(review_id)
        REFERENCES review_table(review_id)
);

CREATE TABLE movie_genre_table(  -- Carola
    movie_id INT NOT NULL,
    genre_id INT NOT NULL,
    CONSTRAINT PK_movie_genre_table PRIMARY KEY(
        movie_id,
        genre_id
    ),
